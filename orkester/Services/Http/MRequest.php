...
class MRequest
{
    ...
    public function resolveFormat()
    {
        ...
        if (strpos($accept, "application/xhtml") !== false || strpos($accept, "text/html") !== false || substr($accept,
                0, 3) == "*/*") {
            $this->format = "html";
            return;
        }
    ...
    public function isAjax()
    {
        ...
        $value = MArrayHelper::getValue($_SERVER, 'HTTP_X_REQUESTED_WITH');
        if ($value === null){
            return false;
        }
    ...
    {
        if (isset($_REQUEST)) {
            if (array_key_exists('__ISFILEUPLOAD', $_REQUEST)) {
    ...
    public function getPreferredLanguage()
    {
        if ($this->_preferredLanguage === null) {
            if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) && ($n = preg_match_all('/([\w\-_]+)\s*(;\s*q\s*=\s*(\d*\.\d*))?/',
                    $_SERVER['HTTP_ACCEPT_LANGUAGE'], $matches)) > 0) {
                $languages = array();
                for ($i = 0; $i < $n; ++$i) {
